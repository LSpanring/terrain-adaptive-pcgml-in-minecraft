...
@cloup.option_group(
    ...
    cloup.option("--lr",                     type=float, default=0.0002,            help="Adam learning rate for generator."),
    cloup.option("--lr-d",                   type=float, default=None,              help="Adam learning rate for discriminator. Defaults to lr * 0.1 if not set."),
    ...
    cloup.option("--lambda-l1",              type=float, default=0.0,               help="L1 regularization lambda."),
    cloup.option("--d-down-conv-count",      type=int,   default=4,                 help="Number of down-convs in D."),
    # CHANGE: warmup epochs before adversarial training begins
    cloup.option("--warmup-epochs",          type=int,   default=5,                 help="Epochs to train generator on L1 only before starting adversarial training."),
    # CHANGE: generator steps per discriminator step
    cloup.option("--g-steps",                type=int,   default=2,                 help="Generator optimizer steps per discriminator step."),
)
@cloup.option_group(
    "Initialization",
...
@cloup.option_group(
    ...
    cloup.option("--wandb-name",             type=str,                 help="Wandb run name."),
...
def vox2vox(
    ...
    # CHANGE: default discriminator LR to 1/10th of generator LR if not explicitly set
    if lr_d is None:
        lr_d = lr * 0.1
    ...
    print(f"Generator LR: {lr}, Discriminator LR: {lr_d}, Warmup epochs: {warmup_epochs}, G steps per D step: {g_steps}")
    ...
    if os.path.exists(f"{output_dir}/metrics.csv"):
        ...
        metricsFile.write("epoch,batch,loss-g,loss-g-gan,loss-g-l1,loss-d,loss-d-real,loss-d-fake\n")
    ...
    class Vox2VoxLearner(Learner):
        def __init__(self):
            ...
            # CHANGE: separate learning rates for G and D
            self.optimizerG = torch.optim.Adam(self.generator    .parameters(), lr=lr,   betas=(beta1, beta2))
            self.optimizerD = torch.optim.Adam(self.discriminator.parameters(), lr=lr_d, betas=(beta1, beta2))
        ...
        def trainingStep(self, batch):
            ...
            # CHANGE: warmup gate — no adversarial signal for the first warmup_epochs epochs
            adversarial_active = self.epochIndex >= warmup_epochs

            # CHANGE: compute one detached fake before any optimizer step so the
            # discriminator always trains against the pre-update generator
            with torch.no_grad():
                bFake_detached = self.generatorActivation(self.generator(a).detach())
                if binary:
                    bFake_detached = torch.sign(bFake_detached)

            # CHANGE: run g_steps generator updates per discriminator update
            for _ in range(g_steps):
                bFake    = self.generatorActivation(self.generator(a))
                predFake = self.discriminator(a, bFake)

                if adversarial_active:
                    lossGan = self.lossGan(predFake, True)
                else:
                    lossGan = torch.tensor(0.0, device=a.device)

                lossL1 = self.lossL1(bFake, bReal) * lambda_l1
            ...
            if adversarial_active:
                ...
                predFake = self.discriminator(a, bFake_detached)
                ...
                lossD    = torch.tensor(0.0, device=a.device)
                lossReal = torch.tensor(0.0, device=a.device)
                lossFake = torch.tensor(0.0, device=a.device)

            metricsFile.write(f"{self.epochIndex},{self.batchIndex},{lossG.item()},{lossGan.item()},{lossL1.item()},{lossD.item()},{lossReal.item()},{lossFake.item()}\n")
            if use_wandb and self.batchIndex % wandb_log_freq == 0:
                wandb.log({
                    "loss-g": lossG.item(),
                    "loss-g-gan": lossGan.item(),
                    "loss-g-l1": lossL1.item(),
                    "loss-d": lossD.item(),
...
@cloup.option_group(
    ...
    cloup.option("--lr",                     type=str,   default="0.0002",  help="Adam learning rate for generator. (**)"),
    cloup.option("--lr-d",                   type=str,   default=None,      help="Adam learning rate for discriminator. Defaults to lr * 0.1 per scale if not set. (**)"),
    ...
    cloup.option("--lambda-l1",              type=str,   default="0.0",     help="L1 regularization lambda. (**)"),
    cloup.option("--d-down-conv-count",      type=str,   default="4",       help="Number of down-convs in D. (**)"),
    cloup.option("--warmup-epochs",          type=str,   default="5",       help="Epochs to train generator on L1 only before adversarial training. (**)"),
    cloup.option("--g-steps",                type=str,   default="2",       help="Generator optimizer steps per discriminator step. (**)"),
)
@cloup.option_group(
    "Initialization",
...
def vox2vox_multiscale(
    ...
    lambda_l1s             = [float(x) for x in lambda_l1         .split(",")]
    d_down_conv_counts     = [int(x)   for x in d_down_conv_count .split(",")]
    warmup_epochs_list     = [int(x)   for x in warmup_epochs     .split(",")]
    g_steps_list           = [int(x)   for x in g_steps           .split(",")]
    ...
    # CHANGE: parse lr_d, defaulting to lr * 0.1 per scale
    if lr_d is not None:
        lr_ds = [float(x) for x in lr_d.split(",")]
    else:
        lr_ds = [l * 0.1 for l in lrs]

    for l in [epoch_counts, batch_sizes, lrs, lr_ds, beta1s, beta2s, lambda_l1s, d_down_conv_counts, warmup_epochs_list, g_steps_list, save_epoch_freqs]:
        if len(l) == 1:
            l *= scaleCount
        if len(l) != scaleCount:
            eprint(
    ...
    for scale in tqdm(range(initial_scale, scaleCount), desc="Scale", total=scaleCount, initial=initial_scale):
        ...
        if use_wandb:
            ...
            wandb.init(
                ...
                entity=wandb_entity,
                project=wandb_project,
                name=f"{wandb_name} (scale {scale})" if wandb_name is not None else None,
                group=modelName,
                config={
                    "model":              modelName,
                    "scale":              scale,
                    ...
                    "warmup-epochs":      warmup_epochs_list,
                    "g-steps":            g_steps_list,
                    "continue":           continue_,
                },
                dir=f"{output_dir}"
            )
        ...
        vox2vox_multiscale_train_single_scale(
            ...
            warmup_epochs =       warmup_epochs_list[scale],
            g_steps =             g_steps_list[scale],
...
def vox2vox_multiscale_train_single_scale(
    ...
    class Vox2VoxLearner(Learner):
        ...
        def trainingStep(self, batch):
            ...
            # Pass input image through previous generators
            with torch.no_grad():
            ...
            # CHANGE: compute detached fake before any optimizer step
            with torch.no_grad():
                bFake_detached = self._applyGenerator(self.generator, downscaledA, upscaledPrevGenOut).detach()

            # CHANGE: g_steps generator updates per discriminator update
            for _ in range(g_steps):
            ...
            if adversarial_active:
                ...
                predFake = self.discriminator(downscaledA, bFake_detached)
                lossReal = self.lossGan(predReal, True)
                lossFake = self.lossGan(predFake, False)
                lossD    = 0.5 * (lossReal + lossFake)
