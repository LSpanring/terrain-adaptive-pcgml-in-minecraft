...
def getTerrainBinaryGdpc(size: ivec3, bottomY: int, remove_flow: bool, offsetXZ: ivec2 = ivec2(0,0)):
    eprint("Setting up gdpc editor and loading ring-adaptive_vox2vox-categorical_L1-0_2x2_epoch-5 slice...")
    ...
    genBox.offset += ivec3(offsetXZ.x, 0, offsetXZ.y)
    editor.loadWorldSlice(genBox.toRect(), heightmapTypes=None, cache=True)
...
def getTerrainEmbeddingGdpc(size: ivec3, bottomY: int, palette: Palette, embeddings: np.ndarray, prune_auto_states: bool, remove_flow: bool, fix_1_16_palette: bool, offsetXZ: ivec2 = ivec2(0,0)):
    eprint("Setting up gdpc editor and loading ring-adaptive_vox2vox-categorical_L1-0_2x2_epoch-5 slice...")
    editor = Editor(buffering=True, bufferLimit=2048)
    buildBox = editor.getBuildArea()
    buildRect = buildBox.toRect()
...
@cloup.pass_context
@cloup.option("--target", type=str, default="gdpc", help="Method and/or ring-adaptive_vox2vox-categorical_L1-0_2x2_epoch-5 to use as target. Must be \"gdpc\" (GDPC) or a ring-adaptive_vox2vox-categorical_L1-0_2x2_epoch-5 path (Amulet).")
...
@cloup.option("--rows", type=int, default=1, help="Number of rows in the grid")
@cloup.option("--cols", type=int, default=1, help="Number of columns in the grid")
def cli(ctx: cloup.Context, target: str, offset: Tuple[int, int], bottom_y: int, size: Tuple[int, int, int], rows: int, cols: int):
    ...
    def runWithModel(model: Callable[[torch.Tensor], torch.Tensor]):
        for row in range(rows):
            for col in range(cols):
                currentOffsetXZ = offsetXZ + ivec2(col * size.x, row * size.z)
                if target == "gdpc":
                    terrain, genBox, editor = getTerrainBinaryGdpc(size, bottomY, remove_flow, currentOffsetXZ)
                else:
                    terrain, genBox = getTerrainBinaryAmulet(target, currentOffsetXZ, size, bottomY, remove_flow)

                terrainTensor = (torch.from_numpy(terrain).float() * 2 - 1).unsqueeze(0).to(device)

                outputTensor = model(terrainTensor)

                eprint(f"Preparing output for placement (row {row}, col {col})...")
    ...
    def runWithModel(model: Callable[[torch.Tensor], torch.Tensor]):
        ...
        for row in range(rows):
            for col in range(cols):
                currentOffsetXZ = offsetXZ + ivec2(col * size.x, row * size.z)
                if target == "gdpc":
                    terrain, genBox, editor = getTerrainEmbeddingGdpc(size, bottomY, palette, embeddings, prune_auto_states, remove_flow, fix_1_16_palette, currentOffsetXZ)
                else:
                    terrain, genBox = getTerrainEmbeddingAmulet(target, currentOffsetXZ, size, bottomY, palette, embeddings, prune_auto_states, remove_flow, fix_1_16_palette)
