...
def train(
    ...
    if device.type == "cuda":
        ...
        eprint(f"Using device: {deviceName} ({gpu_name})")
    else:
        eprint(f"Using device: {deviceName}")
    ...
    eprint()
    learner.fit(trainingData=dataloader, epochCount=epochCount, initialEpoch=initialEpoch)
